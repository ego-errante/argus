...
- **Prize:** 5,000 USDG (1st 2,500 / 2nd 1,500 / 3rd 1,000). ~45 submissions competing for 3 slots — build to **win**, not just complete.
...
- [ ] Confirm Nigeria eligibility. _(regional listing — verify before submitting.)_
- [x] Claim SolInfra credits (RPC + Yellowstone gRPC); confirm mainnet endpoints + auth.
- [x] Create + fund dedicated mainnet keypair (~0.05 SOL); ensure it's gitignored.
- [x] Decide default payload instruction (self-transfer + Memo is the default; builder is pluggable).
- [x] Get an OpenRouter API key (`OPENROUTER_API_KEY`); confirm the default model slug + any rotation fallbacks (ADR 0006).
